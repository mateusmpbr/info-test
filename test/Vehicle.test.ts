...
describe("Vehicles", () => {
    ...
    describe("Read By Id", () => {
        it("It should read a vehicle with a existing id", async () => {
            const response = await request(app).get("/vehicles/e3988b64-ffc6-4cf5-8d0e-588f0fc3c8da");
            assert.strictEqual(response.status, 200);
        });
    ...
    describe("Update", () => {
        it("It should update an existing vehicle with the correct parameters", async () => {
            const response = await request(app).put("/vehicles/e3988b64-ffc6-4cf5-8d0e-588f0fc3c8da").send({
                placa: "DEF1234",
        ...
        it("It should not update an existing vehicle with placa, chassi or renavam from another vehicle", async () => {
            const response = await request(app).put("/vehicles/fadd3a1d-5828-41d4-b11a-747c789b5a8b").send({
                placa: "DEF1234",
                chassi: "8qs1yhxs0fq5uq7hs",
                renavam: "1123456789",
                modelo: "Tiggo 8",
    ...
    describe("Delete", () => {
        it("It should delete a existing vehicle", async () => {
            const response = await request(app).delete("/vehicles/e3988b64-ffc6-4cf5-8d0e-588f0fc3c8da");
            // Cleaning database.sqlite
            await request(app).delete("/vehicles/fadd3a1d-5828-41d4-b11a-747c789b5a8b");
            // End
            assert.strictEqual(response.status, 200);
        });
